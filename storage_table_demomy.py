...
resourcegroup_name = 'jhu6'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'jhu6'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'eastus'
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'carlist'", select='maker,model,price')
for item in items:
    print('maker: ' + item.maker)
    print('Model: ' + item.model)
    print('price: ' + str(item.price) + '\n')

items = table_service.query_entities('itemstable', filter="PartitionKey eq 'coffeemenu'", select='brand, cupsize,price')
for item in items:
